...
class Neuronio:
    def __init__(self, quantidadeConexoes: int, codigoFuncaoAtivacao: int):
        self._pesos = []  # Lista de pesos que incidirão sobre as entradas
        ...
        self._valorVies = None  # Valor de correção, usada para ajudar a rede em treinamento
    ...
    def realizarFeedForward(self) -> None:
        ...
        for i in range(self._quantidadeConexoes):
            soma += self._pesos[i] * self._entrada[i]  # Soma ponderada das entradas multiplicadas pelos pesos
        soma += self._valorVies  # Soma à bias
